...
Navigate to the mamba directory and install the package. This step ensures all necessary components are correctly installed.
...
Note: Installing from source (provided `mamba`) can help prevent package issues and ensure compatibility between different dependencies. It is recommended to follow these steps carefully to avoid potential conflicts.
...
You can download the pre-trained model weights from [here](https://drive.google.com/file/d/1ZB9iRS-CckRAVjyqwcR66Fh1CHdlRHJw/view?usp=sharing), and make a folder named `model_weight` then put the weight file under the folder. 
